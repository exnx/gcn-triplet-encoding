...
#%%    
class GraphEncoder_RGBStridedDecoder(nn.Module):
    def __init__(self,opt):
        super(GraphEncoder_RGBStridedDecoder, self).__init__()
        self.opt = opt
        ...
        self.proj_rela = nn.Sequential(*[nn.Linear(8,1000), nn.ReLU(), nn.Dropout(0.5)])
        self.gnn = GNN(opt)
...
#        rela_masks = rela_masks.unsqueeze(-1)
        ...
        #get the embeddings 
        obj_embed = self.obj_embed(obj_labels)  # obj label should be a one hot
