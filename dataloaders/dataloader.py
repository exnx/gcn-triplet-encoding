...
class RICO_ComponentDataset(Dataset):
    ...
    def __init__(self,opt, transform):
        ...
        # this will need to be replaced with obj_feats dir, output of object detection
        ...
        self.sg_geometry_dir = 'graph_data/geometry-directed/'  # this is from the build_geometry_graph.py
...
#        ix = index #self.split_ix[index]
        ...
        # get the graph for the index, the img/graph number
    ...
    def get_graph_data(self, index):
        ...
        '''
        Retrieves the graph associated with index num. Geometry graph stored by .np file by index num

        Change so that visual features also retrieved by image number, with size N x dims_out
...
#        sg_use = np.load(self.sg_data_dir + image_id + '.npy', encoding='latin1', allow_pickle=True)[()]
        ...
        # this is probably where I need to add in feats of nodes, call it visual_feats
        # and pull it up from pickle file by index, the store in sg_data
...
#        sg_data['obj_labels'] = np.zeros([len(obj_batch), max_att_len, self.opt.num_obj_label_use], dtype = 'int')
        ...
        # rela
        # for batching, mask shows what's relevant
...
class BlobFetcher():
    ...
    looks like this is what grabs the data
